<template>
  <div>
    <a-card>
      <a-button type="primary" style="margin:10px 0" @click="clickEdit(null)">添加</a-button>
      <a-table :data-source="data">
        <a-table-column key="projectName" title="课题名" data-index="projectName" align="center" />
        <a-table-column key="projectDescribe" title="课题表述" data-index="projectDescribe" align="center" />
        <a-table-column key="projectFromId" title="课题来源" data-index="projectFromId" align="center">
          <template slot-scope="text">
            <span v-if="text == 1">自定选题</span>
            <span v-if="text == 2">社会实践</span>
            <span v-if="text == 3"></span>
            <span v-if="text == 4"></span>

          </template>
        </a-table-column>
        <!-- 每一列的显示控制 -->
        <a-table-column key="projectState" title="选题状态" data-index="projectState" align="center">
          <!-- 列里边的模板，text代表该行的列数据,record代表该行整体对象数据，这里record.projectStatus就等于text -->
          <template slot-scope="text">
            <a-tag v-if="text == 0" color="blue">
              未选择
            </a-tag>
            <a-tag v-if="text == 1" color="orange">已被选择，待审核</a-tag>
            <a-tag v-if="text == 2" color="red">已完成选择</a-tag>
          </template>
        </a-table-column>
        <a-table-column key="studentName" title="选题学生" data-index="studentName" align="center" />
        <!-- 不写template模板的话，直接显示该行数据对应的key配对的值 -->
        <a-table-column key="teacherName" title="指导老师" data-index="teacherName" align="center" />
